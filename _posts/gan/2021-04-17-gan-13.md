...
excerpt: "Conditional 하게 이미지를 합성하는 Auxiliary Classfier GANs, ACGAN model에 대해 알아본다."
...
> ✍🏻 이번 포스팅에서는 Conditional 하게 이미지를 합성하는 Auxiliary Classfier GANs, **ACGAN model**에 대해 살펴본다.
...
<font color="gray" font-size = "small"><i> (출처) Naver AI Lab 최윤제 연구원님 발표자료  </i></font>

ACGAN의 
- Generator는 noise를 sampling할 때 class label도 같이 sampling한다. 
$$X_{\text {fake }}=G(c, z)$$
- Discriminator 역시 source와 class label의 확률분포를 바탕으로 학습을 한다.
$$P(S \mid X), P(C \mid X)=D(X)$$

**Object Function**
- log likelihood of the correct source $L_s$
$$\begin{array}{r}
L_{S}=E\left[\log P\left(S=\text { real } \mid X_{\text {real }}\right)\right]+ \\
\qquad E\left[\log P\left(S=\text { fake } \mid X_{\text {fake }}\right)\right]
\end{array}$$

- log likelihood of the correct class $L_c$
$$\begin{array}{r}
L_{C}=E\left[\log P\left(C=c \mid X_{\text {real }}\right)\right]+ \\
E\left[\log P\left(C=c \mid X_{\text {fake }}\right)\right]
\end{array}$$

> D는 $L_s + L_c$를, G는 $L_c - L_s$를 최대화하는 방향으로 학습하며 class label과 noise `z`는 독립적으로 학습한다.
> 이렇게 GAN을 바꿈으로써 좋은 결과를 냈고, training을 안정화시켰다.

<span style='background-color: #E5EBF7;'> real/fake 쪽을 판별하는 부분은 G와 D가 적대적으로 학습을 하지만, class prediction 쪽은 하지 않게 학습한다. </span>

또한, 이전 연구들은 class의 수를 늘리면 quality가 줄어들었지만, ACGAN은 class별로 큰 데이터셋을 나눈 후 각 subset에 대해 G와 D를 학습할 수 있기 때문에 안정적이다. 
...
<p align='center'><img src='https://github.com/happy-jihye/happy-jihye.github.io/blob/master/_posts/images/gan/acgan3.PNG?raw=1' width = '700' ></p>
따라서 생성모델을 평가하려면 다양성을 측정하는 것이 중요하다. 본 논문에서는 MS-SSIM(0~1)의 method를 사용하여 모델의 다양성을 평가한다. 다양성이 높을수록 MS-SSIM score가 낮게 나온다.

CGAN에서는 기존의 GAN과는 다르게 높은 해상도의 이미지를 생성할수록 다양한 이미지가 생성된다. (mode collapse에 빠질 확률이 줄어듦)

---

<p align='center'><img src='https://github.com/happy-jihye/happy-jihye.github.io/blob/master/_posts/images/gan/acgan4.PNG?raw=1' width = '700' ></p>

기존의 SoTA model보다 이미지가 잘 생성된다.

## 4. Discussion

> 고해상도의 이미지는 class에 대한 정보를 제공하여 discriminability 훈련이 더 잘되며, high-resolution의 이미지를 만들수록 다양한 이미지를 생성할 확률이 높다. 본 논문은 100개의 class를 가진 ImageNET dataset에 대해서도 `128 x 128`의 이미지를 합성할 수 있음을 보여주었다.
