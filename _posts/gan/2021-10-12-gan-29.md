...
<p align='center'><img src='https://github.com/happy-jihye/happy-jihye.github.io/blob/master/_posts/images/gan/face-vid2vid-1.jpeg?raw=1' width = '700' ></p>
...
<p align='center'><img src='https://github.com/happy-jihye/happy-jihye.github.io/blob/master/_posts/images/gan/face-vid2vid-2.jpeg?raw=1' width = '700' ></p>
<p align='center'><img src='https://github.com/happy-jihye/happy-jihye.github.io/blob/master/_posts/images/gan/face-vid2vid-3.jpeg?raw=1' width = '700' ></p>

#### Training

- Loss function: 다음 loss를 minimize하는 방향으로 학습

$$\begin{aligned}
&\mathcal{L}_{P}(d, y)+\mathcal{L}_{G}(d, y)+\mathcal{L}_{E}\left(\left\{x_{d, k}\right\}\right)+ \\
&\mathcal{L}_{L}\left(\left\{x_{d, k}\right\}\right)+\mathcal{L}_{H}\left(R_{d}, \bar{R}_{d}\right)+\mathcal{L}_{\Delta}\left(\left\{\delta_{d, k}\right\}\right)
\end{aligned}$$

- $\mathcal{L}_{P}(d, y)+\mathcal{L}_{G}(d, y)$ : output image가 ground trouth랑 비슷하도록 학습
  - `Perceptual Loss` $\mathcal{L}_{P}$ : perceptual loss를 minimize하는 방향으로 학습하여 driving image와 비슷하면서 sharp하게 보이는 output image를 학습
  - `GAN Loss` $\mathcal{L}_{G}$ :  / discriminator가 patch 단위로 판별하는 multi-resolution patch GAN([`patch-GAN 설명`](https://happy-jihye.github.io/gan/gan-8/#322-markovian-discriminator-patchgan))을 사용 & discriminator feature matching loss를 최소화하는 방향으로 학습

- $\mathcal{L}_{E}\left(\left\{x_{d, k}\right\}\right)+ \mathcal{L}_{L}\left(\left\{x_{d, k}\right\}\right)$ : keypoint가 consistent하게 예측되도록 학습
  -  `Equivariance Loss` $\mathcal{L}_{E}$ : image-specific keypoint $x_{d,k}$ 의 consistency를 보장, 이 모델은 3D keypoint를 사용하므로 이미지에서 2D transformation을 하고 싶다면 이미지를 평면에 orthographic projection한 다음에 transformation을 해야함
  -  `Keypoint prior loss` $\mathcal{L}_{L}$ : Keypoint converge loss를 사용하여 $x_{d,k}$ 가 국소적인 영역에 모여있지 않고 얼굴을 전반적으로 추정하도록 도와줌 (keypoint의 쌍이 너무 가까우면 penalty를 부여)

- `Head pose loss` $\mathcal{L}_{H}\left(R_{d}, \bar{R}_{d}\right)$ : pre-trained pose estimation network([`natanielruiz/deep-head-pose`](https://github.com/natanielruiz/deep-head-pose))를 이용해서 $R_{d}$ 가 ground truth $\bar{R}_{d}$ 와 비슷하도록 학습
  
- `Deformation prior loss` $\mathcal{L}_{\Delta}\left(\left\{\delta_{d, k}\right\}\right)$ : $\delta_{d, k}$ 가 작도록 penalty를 부여

<span style='background-color: #E5EBF7;'> **Experiments** </span>

- **Dataset**: `VoxCeleb2`, `TalkingHead-1KH`
- **Comparison**: `FOMM`, `few-shot vid2vid`, `bi-layer model`
