...
- <span style='background-color: #FFF2CC;'> **A-block** (learned affine transform) </span> : style code인 $w$를 뽑고나면, [affine transformation](https://ko.wikipedia.org/wiki/%EC%95%84%ED%95%80_%EB%B3%80%ED%99%98)을 통해 $w$를 style $y$로 바꿔준다. 

  (아핀 변환은 일종의 fc layer이며, 위의 모델에서 A-block은 각기 다른 파라미터를 가진 fc layer)

- style $\mathbf{y}=\left(\mathbf{y}_{s}, \mathbf{y}_{b}\right)$ 
