...
> StyleGAN은 이러한 문제를 해결하고자 latent vector `z`를 generator의 input으로 사용하지 않고, **latent vector를 non-linear mapping network에 넣어 disentanglement**하게 만들고자 하였다. (각 feature들이 상관관계를 갖지 않도록 보정함)
