...
<span style='background-color: #C6FED9;'> 또한, $F(G(X)) \approx X$ 의 식이 만족하도록 **cycle consistency loss** 를 도입하였으며, 이 모델은 style transfer, object transfiguration, season transfer, photo enhancement 등 다양한 task의 image-to-image 변환을 가능하게 했다. </span>
...
또한, <span style='background-color: #E5EBF7;'> $F(G(x)) \approx x \text$, $G(F(y)) \approx y$의 **cycle consistency loss** </span>도 추가하였다.
...
또한, $\mathcal{L}_{\mathrm{GAN}}\left(G, D_{Y}, X, Y\right) &=\mathbb{E}_{y \sim p_{\text {data }}(y)}\left[\log D_{Y}(y)\right]+\mathbb{E}_{x \sim p_{\text {data }}(x)}\left[\log \left(1-D_{Y}(G(x))\right]\right$ 의 negative log likelihood loss를 **Least-squares Loss**로 바꿨다.
