...
<span style='background-color: #E5EBF7;'> **Consistency Regularization Loss** </span>

$$L_{c r}=\|D(x)-D(T(x))\|^{2}$$
- $T(x)$ : stochastic data augmentation funtion

<span style='background-color: #E5EBF7;'> **CR-GAN objective function** </span>

$$L_{D}^{c r}=L_{D}+\lambda L_{c r}, \quad L_{G}^{c r}=L_{G}$$
- $L_{G}, L_{D}$는 GAN Loss 
  - original GAN
