...
2. content와 motion이 disentangle한 상태에서 원하는 latent trajectory를 찾을 수 있는 <b>motion generator</b> 발견
...
#### LSTM network
- `Motion Generator`: 두 개의 `LSTM network(LSTM encoder, LSTM decoder)`를 이용하여 latent motion <i>trajectory</i> $\mathbf{Z}=\left\{\mathbf{z}_{1}, \mathbf{z}_{2}, \cdots, \mathbf{z}_{n}\right\}$를 예측
  - $n$: number of frames
...
- 다만 $\mathcal{L}_{D_{\mathrm{V}}}$ loss 만을 이용하여 학습을 하면, $\tilde{\mathbf{x}}_{t|t>1}$ 가 첫번째 생성 이미지 $\tilde{\mathbf{x}}_{1}$과 identity(content)가 달라질 수도 있음 ➡ <b>Contrastive Image Discriminator</b>를 도입 !

### 2.2 Contrastive Image Discriminator $D_I$

> ${\mathbf{x}}_{t|t>1}$과 $\tilde{\mathbf{x}}_{1}$간의 content와 image quality가 match 되는지를 판별
