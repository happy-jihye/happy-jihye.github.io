...
> 🤔 결과가 굉장히 인상적인 모델이 나왔다. 올해 포스텍에서 발표한 `BDInvert`라는 모델인데, align이 맞지 않는 이미지들에 대해서도 inversion 및 reconstruction을 잘한다. 아직 Image editing 성능에는 제약이 있어보이지만, alignment를 강하게 걸어야하는 stylegan-based model을 align이 안된 이미지들에 대해서도 잘 생성되도록 만들었다는 점에서 아주 impressive한 모델인 것 같다.
