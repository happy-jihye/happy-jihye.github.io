...
그동안 배웠던 layer를 살펴봅시다. 

데이터 입력이 들어오면, FC나 Conv layer에서 가중치곱을 하고 그 결과값을 활성함수에 넣어 **비선형 연산**을 합니다.
...
sigmoid function은 입력값을 0과 1사이의 값으로 바꿔주며, 입력값이 크면 출력은 1에 가깝고 입력값이 작으면 출력은 0에 가깝습니다. 또한, 0부근은 선형 함수와 비슷합니다.
...
> 1. <u>Saturation되는 것이 gradient를 없앱니다.</u>
>
...
>
>    -  Sigmoid function을 computation graph로 보면, back-propagation의 과정에서 chain-rule이 적용됩니다. 근데 x는 -10, 10처럼 매우 작거나 큰 값이라면 gradient는 0이 되고 gradient는 소실되게 됩니다.
>
> 2. <u>sigmoid의 출력값이 zero-centered하지 않습니다.</u>
>
>    - 만약 뉴런의 입력이 항상 양수라면 gradient는 항상 양수거나 음수가 될 것입니다.(늘 같은 방향으로 움직임)
>
>    - 그렇다면 파라미터를 업데이트 할 때 항상 증가하거나 감소할 수 밖에 없습니다.
>
>    - <img src="image/lec6/image-20210329170148743.png" alt="image-20210329170148743" style="zoom:50%;" />
>
>    -  만약의 위의 예제에서 파란색 선이 최적의 가중치 vector(w)라고 할때, gradient는 이 방향으로 학습을 할 수 없게 됩니다.(1/3사분면에서만 학습이 됨)
>
>    - 그렇기 때문에 빨간색 화살표처럼 여러번의 gradient 업데이트를 수행하여 학습을 해야합니다.
>
>      (빨간색 화살표는 gradient가 이동가능한 방향)
>
> 3. <u>exp()계산으로 인해 computation 이 어렵습니다</u>
...
다음은 sigmoid의 **zero-centered** 문제를 개선한 **tanh**함수입니다. 이 함수는 sigmoid보다는 낫지만 saturation문제를 해결하지 못했다는 단점이 있습니다.
...
>
>   <p align="center"><img title = "image-20210329171452704" src="https://github.com/happy-jihye/happy-jihye.github.io/blob/master/_posts/cs231n/images/lec6/image-20210329171452704.png?raw=1" width = "600" ></p>{: width="50%", height="50%"}{: .center}
>
...
ReLU를 변형시킨 이 버전은 dying problem도 없으며, staturation도 되지 않습니다.
...
가장 대표적인 전처리 과정은 **zero-mean**으로 만든 후 **normalize**하는 겁이다.

- **zero-mean**을 하는 이유는 모든 입력이 양수이거나 음수인 경우에 gradient 역시 항상 양수이거나 음수여서 잘 학습되지 않는 경우를 피하기 위해서입니다.
- **normalization**을 하는 이유는 모든 차원이 동일한 범위 안에 있게 해줘서 전부 동등하게 기여를 하게 하기 위해서입니다.
...
<p align="center"><img title = "image-20210329174654805" src="https://github.com/happy-jihye/happy-jihye.github.io/blob/master/_posts/cs231n/images/lec6/image-20210329174654805.png?raw=1" width = "600" ></p>
...
- 만약에 모든 weight vector를 0으로 초기화하면 모든 뉴런은 같은 연산을 수행할 것이고, 출력과 gradient도 서로 같을 것입니다.

  즉, 모든 가중치가 같은 값으로 업데이트 되기 때문에 옳게 학습되지 못합니다.

   

**1) Small random number**로 초기화 (gaussian with zero mean and 1e-2 standard deviation)

- 작은 네트워크에서는 괜찮지만, 네트워크가 깊어질수록 문제가 생깁니다.
...
- 위의 layer에서는 tanh activation function을 사용하는데, tanh이 zero-centered이므로 <u>평균이 항상 0 근방</u>에 있습니다. 

- 초기 layer에서는 가우시안 분포를 잘 따르지만, layer가 깊어질 수록 이 분포가 무너집니다. 

  가중치값이 너무나도 작은 값이라서 W를 곱하면 곱할 수록 출력값이 급격히 줄어드는 것입니다.

- backward를 하는 경우에도 forward와 유사합니다. 가중치를 업데이트 하는 과정에서 Wx를 W에 대해 미분하면 x가 되는데 x역시 엄청 작은 값이므로 이를 upstream gradient와 곱해주면 최종 gradient가 매우 작아져서 update가 잘 안될 것입니다.
...
이 문제를 해결하기 위한 논문도 있습니다. 이 논문에서는 scaling할 때 추가적으로 2를 더 나눠줌으로써 뉴런 중 절반이 날라가는 걸 예방합니다.
...
즉, <u>어떤 layer로부터 나온 batch단위 만큼의 activation이 있을 때 이 값을 unit gaussian으로 만들어주기 위해 현재 batch에서 계산한 mean과 variance로 normalize를 시키는 것입니다.</u>
...
그리고 <u>이 과정을 FC / Conv Layer 이후에 하도록 해서 deep network에서 W가 지속적으로 곱해져서 **bad scaling effect**가 발생하는 것을 막습니다.</u>

Batch Normalization은 입력의 스케일만 살짝씩 조정해주는 역할을 하기 때문에 FC나 Conv어디에든 적용할 수 있습니다.

- Conv Layer에서는 BN을 약간 다르게 해줍니다. Conv layer에서는 normalization을 차원마다 독립적으로 수행하지 않고, 같은 Activatoin map의 같은 channel에 있는 요소들을 한번에 normalize합니다.
...
근데 사실 중요한 것은 saturation이 전혀 일어나지 않는 것이 아니라, "얼마나" saturation이 일어날지를 조정해주는 것입니다. 
...
학습을 할 때 데이터가 작으면 overfit이 될 것이고 Loss가 매우 줄어듭니다. 

이 경우에는 regularization을 하지 않고 loss가 줄어드는 걸 확인합니다.
...
이번에는 전체 데이터를 사용하고, regularization을 사용하면서 다양한 learning rate로 실험을 해봤습니다.

만약 learning rate가 너무 작다면, gradient 업데이트가 충분히 일어나지 않게 되고 cost가 잘 안변합니다.
...
1. **coarse stage** :먼저 처음에는 epoch를 몇번 돌려서 넓은 범위에서 값을 골라냅니다.
   ...
   NaN이 뜨거나 Loss가 줄지 않거나 하는 걸 보면서 parameter를 조정합니다.
...
<p align="center"><img title = "image-20210329221259768" src="https://github.com/happy-jihye/happy-jihye.github.io/blob/master/_posts/cs231n/images/lec6/image-20210329221259768.png?raw=1" width = "600" ></p>

실험을 여러번 해서 최적의 hyper-parameter를 찾아야합니다.

<p align="center"><img title = "image-20210329221426624" src="https://github.com/happy-jihye/happy-jihye.github.io/blob/master/_posts/cs231n/images/lec6/image-20210329221426624.png?raw=1" width = "600" ></p>

다음과 같은 경우는 gradient의 backpropgation이 초기에는 잘 안되다가 학습이 진행되면서 회복이 되는 경우로, 별로 좋지는 않은 상황입니다.
